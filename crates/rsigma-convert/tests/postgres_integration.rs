...
const EVENTS: &[(&str, &str)] = &[
    ...
    ("2023-08-15T13:55:00Z", r#"{"eventType":"user.session.start","actor":{"alternateId":"bob@acme.com","displayName":"Bob Smith","type":"User"},"outcome":{"result":"SUCCESS"},"client":{"ipAddress":"192.168.1.100"},"securityContext":{"isProxy":false}}"#),
    // 2. Attacker proxy session (detection: proxy rule)
    ("2023-08-15T14:05:00Z", r#"{"eventType":"user.session.start","actor":{"alternateId":"superadmin@acme.com","displayName":"IT Admin","type":"User"},"outcome":{"result":"SUCCESS"},"client":{"ipAddress":"198.51.100.23"},"securityContext":{"isProxy":true}}"#),
    // 3. MFA deactivation (detection: MFA rule)
    ("2023-08-15T14:12:00Z", r#"{"eventType":"user.mfa.factor.deactivate","actor":{"alternateId":"superadmin@acme.com","displayName":"IT Admin","type":"User"},"outcome":{"result":"SUCCESS"},"client":{"ipAddress":"198.51.100.23"},"securityContext":{"isProxy":true}}"#),
    // 4. Ops team app update (noise, no detection)
    ("2023-08-15T14:15:00Z", r#"{"eventType":"application.lifecycle.update","actor":{"alternateId":"ops@acme.com","displayName":"Ops Team","type":"User"},"outcome":{"result":"SUCCESS"},"client":{"ipAddress":"10.0.0.50"},"securityContext":{"isProxy":false}}"#),
    // 5. Admin privilege grant (detection: admin role rule)
    ("2023-08-15T14:18:00Z", r#"{"eventType":"user.account.privilege.grant","actor":{"alternateId":"superadmin@acme.com","displayName":"IT Admin","type":"User"},"outcome":{"result":"SUCCESS"},"client":{"ipAddress":"198.51.100.23"},"securityContext":{"isProxy":true}}"#),
    // 6. Rogue IdP created (detection: IdP rule)
    ("2023-08-15T14:25:00Z", r#"{"eventType":"system.idp.lifecycle.create","actor":{"alternateId":"superadmin@acme.com","displayName":"IT Admin","type":"User"},"outcome":{"result":"SUCCESS"},"client":{"ipAddress":"198.51.100.23"},"securityContext":{"isProxy":true}}"#),
];
...
async fn setup_db() -> (testcontainers::ContainerAsync<Postgres>, tokio_postgres::Client) {
...
async fn proxy_session_default_format() {
    ...
    assert_eq!(results[0].1, 1, "only the attacker proxy session should match");
}
...
async fn mfa_deactivated_default_format() {
    ...
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].1, 1, "only the MFA deactivation event should match");
}
...
async fn admin_role_default_format() {
    ...
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].1, 1, "only the privilege grant event should match");
}
...
async fn view_format_creates_queryable_view() {
    ...
    let rows = client
        .execute("SELECT * FROM sigma_bde30855_5c53_4c18_ae90_1ff79ebc9578", &[])
        .await
...
async fn all_four_rules_together() {
    ...
    assert_eq!(total, 4, "4 events should match across 4 rules (2 noise events ignored)");
}
...
async fn correlation_event_count_against_postgres() {
    ...
    assert_eq!(results.len(), 2, "should have detection + correlation queries");
    assert_eq!(results[0].1, 1, "detection rule should match 1 proxy session");
    assert!(
        results[1].1 >= 1,
