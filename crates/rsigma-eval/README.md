...
- **Logsource-aware evaluation**: opt-in `LogSourceExtractor` implementations (`FieldLogSourceExtractor` built in) plus conflict-based pruning (`set_logsource_extractor`) skips rules whose `product`/`service`/`category` (and custom dimensions) conflict with the event's logsource, backed by a product-partitioned rule index; off by default and fail-open
