...
Verified Criterion numbers ship in [`BENCHMARKS.md`](https://github.com/timescale/rsigma/blob/main/BENCHMARKS.md). Headline figures: 2 µs per event for 100 rules, 30 µs per event for 1k rules, 162 µs per event for 5k rules. Cross-rule AC delivers up to ~100× speed-up on pure-substring rule sets dominated by non-matching events.
...
- [`BENCHMARKS.md`](https://github.com/timescale/rsigma/blob/main/BENCHMARKS.md) for the Criterion results across parser, evaluator, correlation engine, runtime, and dynamic pipelines.
