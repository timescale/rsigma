...
| `LogSourceExtractor<E>` trait + `FieldLogSourceExtractor` | `LogSourceExtractor<E: Event>` (methods `extract` and `resolve`) derives an event's `LogSource` for conflict-based logsource pruning; `FieldLogSourceExtractor` is the built-in implementation for every event type, reading configurable fields plus optional static defaults. `Engine<L>`, `CorrelationEngine<L>` and `SchemaRouter<L>` are generic over the extractor type `L` (default `FieldLogSourceExtractor`), so extraction is statically dispatched. |
...
| `Engine::set_logsource_extractor(Option<L>)` / `Engine::with_logsource_extractor(L)` | Opt into conflict-based logsource pruning: skip rules whose `product`/`service`/`category` (and custom dimensions) conflict with the event's. Off by default, fail-open. Pays off on large mixed-product rule sets. |
