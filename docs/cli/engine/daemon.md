...
| `--alert-pipeline <PATH>` | YAML file declaring the alert pipeline. It runs after enrichment and before the sinks, deduplicating results by a configurable fingerprint: the first fire passes through and opens an active alert, duplicates fold into it, the alert re-emits on `repeat_interval`, and it resolves after `resolve_timeout`. Hot-reloaded on `SIGHUP`, file-watcher changes, and `POST /api/v1/reload`; failed reloads keep the previous pipeline active. Selector and scope errors reject the daemon at startup. See [Alert Pipeline](../../guide/alert-pipeline.md). |
