...
Lint Sigma rules against the v2.1.0 specification with 68 built-in checks.
...
Thirteen of the 68 rules carry safe auto-fixes; pass `--fix` to apply them in place. Optional JSON schema validation, three suppression tiers (CLI, config file, inline comments), and a tiered `--fail-level` for CI gating.
...
- [Lint Rules reference](../../reference/lint-rules.md) for the complete 68-rule catalog.
